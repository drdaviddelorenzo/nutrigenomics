#!/usr/bin/env python3
"""
generate_patient.py — Random synthetic patient generator for Nutrigenomics demos.

Generates a realistic 23andMe-format CSV file with randomly assigned genotypes
for all SNPs in the Nutrigenomics panel. Each run produces a different risk profile,
...
Output:
    ...
    (or nutrigenomics_results_<seed>/ if --run is used)
"""
...
def generate_csv(seed: int, output_path: Path) -> Path:
    ...
    lines = [
        ...
        f"# SYNTHETIC DATA — generated by Nutrigenomics generate_patient.py (seed={seed})",
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate random synthetic patient file(s) for Nutrigenomics demos"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Immediately run Nutrigenomics on each generated file"
    )
    parser.add_argument(
    ...
    for seed in seeds:
        ...
        if args.run:
            ...
            cmd = [
                sys.executable,
                str(skill_root / "nutrigenomics.py"),
                "--input", str(out_csv),
                "--output", str(out_dir),
            ]
            if args.no_figures:
